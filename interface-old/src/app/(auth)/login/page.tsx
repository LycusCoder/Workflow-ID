  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    if (!isValidEmail(email)) {
      toast({
        variant: "destructive",
        title: "Invalid Email",
        description: "Please enter a valid email address (e.g., user@domain.com)"
      })
      return
    }
    ...
    toast({
      variant: "warning",
      title: "Coming Soon",
      description: "Email login will be available soon. Please use face recognition."
    })
  }
    ...
    try {
      ...
      if (bestMatch.user) {
        ...
        setTimeout(() => {
          setIsScanning(false)
          isScanningRef.current = false
          login(matchedUser) // This will redirect to dashboard
        }, 1500)
      } else {
        ...
        toast({
          ...
          description: "Please register first or try again with better lighting."
        })
        stream.getTracks().forEach(track => track.stop())
        setIsScanning(false)
        isScanningRef.current = false
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to access camera or process face. Please check permissions and try again."
      })
      setIsScanning(false)
      isScanningRef.current = false
