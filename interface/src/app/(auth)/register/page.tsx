    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          ...
          setCountdown(null)
          countdownRef.current = null
          alert("✅ Registration complete! You can now login with face recognition.")
          // Redirect to login
        ...
        alert(`Failed to save face data: ${error.detail || 'Unknown error'}`)
        setIsScanning(false)
        setFaceCaptured(false)
      }
    } catch (error) {
      ...
      alert('Network error while saving face data. Please try again.')
      setIsScanning(false)
      setFaceCaptured(false)
    }
  }
